...
ligands, which we will do with OpenFE. There are a few ways to use OpenFE's tooling - there is a command line interface which provides easy
access to the most commonly-used features and a Python API which enables a wider set of more advanced features and
options.  Here we'll be using the CLI, but there's lots of cool stuff in the Python API too.
...
We will make use of YAML files which encode settings used in these commands. An OpenFE settings file is relatively simple,
...
mapper is used and a minimal spanning network is generated. Open Force Field's Sage force field is used for small molecule parameters, so AM1-BCC
...
We would do this by calling `openfe quickrun` on each JSON file, which itself stores each result in a JSON file. We
don't have an army of GPUs and I don't want to make you wait, so I'm using some pre-computed results.
...
Next, I want to dig deeper into some of these tools we've talked about so far in our high throughput screening pipeline, and then finish up
...
the `settings.yaml` file. Let's try using NAGL with just a single core, as opposed to the explicitly AM1BCC step that just ran with 8. 

_run openfe charge-molecules cells with NAGL options_

We can see that using our new NAGL tool for charge assignment is much faster than explicit AM1BCC, and it is able to assign charges to much larger molecules as well.
...
has implemented a bunch of different networks, and a subset of them have been integrated into OpenFE. The default
...
_run plan-rbfe-network, show new visualization_

There are plenty more options exposed in the OpenFE CLI and documented in the CLI reference, and further more functionality
...
Let's take the ligand which gave us the strongest dG molecule in the earlier portion, ligand 3, which we can load into RDKit and have a quick look at.
...
up into a separate function in the file `simulate.py` if you want to have a look. 

_show lig_3 trajectory_ 

This function returns an NGLview
...
going to run these cells ahead of time so the simulation has time to finish.
...
the small molecule parameters and for the protein we'll use a SMIRNOFF port of AMBER's ff14SB protein force field. One could slot in other force
...
Finally, we could re-use some of that same boilerplate OpenMM code to get an MD simulation out of this state and then
again look at it with NGLview. This function could prepare and run an OpenMM simulation for 30 seconds, but I don't need you to watch it run for 30 seconds, so I'm just loading a run I did earlier.
...
biopolymer loading in OpenFF's infrastructure. A core feature of Pablo is support for custom residue
...
_run atom name correcting cell_

After reaction, we have a single-molecule representation of this modified residue with some atom names changed to match the PDB file.
...
_run next two cells_

A final wrinkle here is that, due to the modified residue, NAGL was used to assign charges
to the protein, although ff14SB charges were backfilled onto standard residues so only the modified cysteine and dye
ended up with NAGl-assigned charges.

Finally, I wanted to demonstrate one of the powerful features of NAGL, which is how quickly it can assign partial
charges to large ligands. 

_show ligand cell_

I've extracted the ligand from the 5FDR PDB record, which is larger than that ligands in the
screening workflow we went through earlier. AM1-BCC becomes much slower as the number of heavy atoms in a molecule
increases.

_highlight AT cell_

Timing I did ahead of time shows that this takes about three minutes and 20 seconds with AmberTools.
...
less than one second (note that the timeit call runs the cells multiple time to accurately measure runtime).
